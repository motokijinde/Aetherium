...
インターネット上のAIサービスには接続せず、お手元のMac上で起動している**Ollama**（Local Server）と**VOICEVOXエンジン**に接続して動作します。
...
## よくあるトラブル
- **モデル/話者が表示されない**: OllamaとVOICEVOXが起動しているか確認してください。
- **読み上げが出ない**: VOICEVOXが起動しているか確認してください。
- **停止したのに次の応答が返らない**: Ollama側が同時接続を制限している場合、停止直後の再送信で待たされることがあります。数秒待って改善しない場合は、Ollamaのログも確認してください。
